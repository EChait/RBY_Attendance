...
$(document).ready(function () {
    ...
    $(".form-check-input123").change(function (e) {
        ...
        window.localStorage.setItem("subjectId", e.target.id);
        window.localStorage.setItem("teacherId", e.target.value);
    })
    ...
    $("#btnAddnewStudent").click(function () {
        ...
        if (newStudent.subjectId != null && newStudent.firstName != null && newStudent.lastName != null && newStudent.teacherId != null)
        {
            $.ajax({
                type: 'POST',
                url: '/NewStudent/Add',
    ...
    $('#achrayusChange').change(function () {
        var nAchrayus = $('#achrayusChange').text();

        $.ajax({
            type: 'POST',
            url: '/Admin/ChangeAchrayus',
            data: JSON.stringify(nAchrayus),
            success: function (data) {
                window.location.reload();
            },
            error: function (err) {
